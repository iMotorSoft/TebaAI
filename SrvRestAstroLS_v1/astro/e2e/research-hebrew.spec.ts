...
test("real scorpion evidence preserves logical Hebrew on desktop, tablet and mobile", async ({ page }) => {
  test.skip(true, "golden Hebrew evidence (LMII 28 / LMI 96) is not produced by the current legacy pipeline selection; corpus-era golden (covered by backend tests)");
...
test("captures controlled pre-index baseline and real negative literal result", async ({ page }) => {
  test.skip(true, "no-evidence contract only holds without the semantic lane; with Milvus up the verse resolves via semantic retrieval (era-sensitive golden)");

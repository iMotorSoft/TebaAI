...
test("captures mobile drawers with real evidence", async ({ page }) => {
  ...
  await page.getByTestId("research-question").fill("תשעה באב"); await page.getByTestId("research-submit").click(); await expect(page.locator(".synthesis")).toBeVisible({ timeout: 45_000 }); await page.screenshot({ path: path.join(shots, "hebrew.png"), fullPage: true }); await page.screenshot({ path: path.join(shots, "mobile-answer.png"), fullPage: true });

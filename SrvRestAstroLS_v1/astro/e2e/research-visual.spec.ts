...
test("captures partial, no-evidence and error states safely", async ({ page }) => {
  ...
  await page.route("**/library/investigative-qa/v1", (route) => fulfillConfirmationPhases(route, { ...mockBase, status: "partial" })); await page.getByTestId("research-question").fill("consulta parcial"); await page.getByTestId("research-submit").click(); await expect(page.getByText("Resultado parcial")).toBeVisible(); await page.screenshot({ path: path.join(shots, "partial.png") });

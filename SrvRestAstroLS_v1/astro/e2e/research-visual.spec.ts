...
test("captures the real desktop research workspace", async ({ page }) => {
  ...
  await page.getByTestId("research-question").fill("¿Dónde aparece la plegaria?"); await page.getByTestId("research-submit").click(); await expect(page.locator(".synthesis")).toBeVisible({ timeout: 30_000 });
  ...
  await page.getByText("Matriz de evidencia", { exact: true }).click(); await page.screenshot({ path: path.join(shots, "desktop-matrix.png"), fullPage: true }); await page.getByText("Cruces entre obras", { exact: true }).click(); await page.screenshot({ path: path.join(shots, "desktop-cross-corpus.png"), fullPage: true });
});
test("captures mobile drawers with real evidence", async ({ page }) => {
  ...
  await page.getByTestId("research-question").fill("מה נאמר על תפילה?"); await page.getByTestId("research-submit").click(); await expect(page.locator(".synthesis")).toBeVisible({ timeout: 30_000 }); await page.screenshot({ path: path.join(shots, "hebrew.png"), fullPage: true }); await page.screenshot({ path: path.join(shots, "mobile-answer.png"), fullPage: true });
...
test("captures partial, no-evidence and error states safely", async ({ page }) => {
  ...
  await page.route("**/library/investigative-qa/v1", async (route) => route.fulfill({ json: { ...mockBase, status: "partial" } })); await page.getByTestId("research-question").fill("consulta parcial"); await page.getByTestId("research-submit").click(); await expect(page.getByText(/respuesta es parcial/)).toBeVisible(); await page.screenshot({ path: path.join(shots, "partial.png") });
  await page.unroute("**/library/investigative-qa/v1"); await page.getByRole("button", { name: /Nueva investigación/ }).first().click(); await page.route("**/library/investigative-qa/v1", async (route) => route.fulfill({ json: { ...mockBase, status: "no_evidence", answer_text: "", answer_markdown: "", summary: "Sin evidencia suficiente" } })); await page.getByTestId("research-question").fill("sin evidencia"); await page.getByTestId("research-submit").click(); await expect(page.getByText(/No se encontró evidencia suficiente/)).toBeVisible(); await page.screenshot({ path: path.join(shots, "no-evidence.png") });
...
test("captures the tablet source drawer", async ({ page }) => {
  test.setTimeout(90_000); await page.setViewportSize({ width: 820, height: 1180 }); await login(page); await page.getByTestId("research-question").fill("¿Qué se dice sobre la tristeza?"); await page.getByTestId("research-submit").click(); await expect(page.locator(".synthesis")).toBeVisible({ timeout: 30_000 }); await page.screenshot({ path: path.join(shots, "tablet-answer.png"), fullPage: true }); await page.getByRole("button", { name: "Fuentes", exact: true }).click(); await page.screenshot({ path: path.join(shots, "tablet-sources.png") });
});

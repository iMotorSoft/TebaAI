<script lang="ts">
  ...
  const activeText = $derived(normalizeDisplayText(active?.display_snippet || active?.paragraph_text || active?.snippet || ""));
        ...
        {#if active.parallel_texts.length}
          <section class="parallel-texts" aria-label="Traducción o ampliación">
            ...
            {#each active.parallel_texts as parallel}
              ...
              <blockquote tabindex="0" lang={parallel.language} dir="ltr">{parallel.text}</blockquote>

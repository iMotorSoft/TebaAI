...
describe("research contract", () => {
  ...
  it("preserves simple RAG status and original query", () => { const r = normalizeResearchResponse({ ...base, pipeline: "simple_rag", research_status: "degraded", original_query: "Relación sangre y habla", retrieval: { semantic_status: "failed", literal_status: "ok" }, processing: { fallback_used: true } }); expect(r.pipeline).toBe("simple_rag"); expect(r.research_status).toBe("degraded"); expect(r.original_query).toBe("Relación sangre y habla"); expect(r.retrieval).toMatchObject({ semantic_status: "failed", literal_status: "ok" }); });
  ...
  it("preserves footnote and printed-reference literal match kinds", () => {
    const footnote = { ...hit("footnote-35", "direct_relation"), work_code: "lh", work_title: "Likutey Halajot — Interior Final", physical_pdf_page: 56, printed_page: 38, source_layer: "footnote", source_layer_confidence: "high", literal_match_kind: "footnote_literal_exact", footnote_marker: "35", quote: "35 Ver §1 más arriba. Escribe el Rabí Natán…" };
    const reference = { ...hit("ref-salmos", "direct_relation"), work_code: "lh", work_title: "Likutey Halajot — Interior Final", physical_pdf_page: 55, source_layer: "editorial_commentary", source_layer_confidence: "high", literal_match_kind: "printed_reference_exact", quote: "37\nSalmos 16:1" };
    const r = normalizeResearchResponse({ ...base, original_query: "El hombre se une a HaShem…", hits: [footnote, reference], claims: [{ claim_id: "footnote", text: "Se encontró la nota", strength: "strong", evidence_ids: ["footnote-35"], primary_evidence_id: "footnote-35" }], primary_evidence_ids: ["footnote-35"] });
    expect(r.hits[0]).toMatchObject({ literal_match_kind: "footnote_literal_exact", footnote_marker: "35", physical_pdf_page: 56, printed_page: 38, source_layer: "footnote" });
    expect(r.hits[1].literal_match_kind).toBe("printed_reference_exact");
  });
  it("preserves structural heading evidence and its associated canonical chunk", () => {
    const source = { ...hit("heading", "direct_relation"), work_code: "lh", work_title: "Likutey Halajot", pdf_page: 51, physical_pdf_page: 51, printed_page: 33, section: "4. CONSTRUYENDO UN MISHKÁN", source_layer: "section_heading", source_layer_confidence: "high", literal_match_kind: "structural_heading_exact", match_kind: "structural_heading_exact", heading_original: "4 ■ CONSTRUYENDO UN MISHKÁN", heading_normalized: "construyendo un mishkán", associated_chunk_id: "body-chunk" };
    const r = normalizeResearchResponse({ ...base, original_query: "CONSTRUYENDO UN MISHKÁN", hits: [source], claims: [{ claim_id: "heading", text: "Se encontró la sección", strength: "strong", evidence_ids: ["heading"], primary_evidence_id: "heading" }], primary_evidence_ids: ["heading"] });
    expect(r.hits[0]).toMatchObject({ literal_match_kind: "structural_heading_exact", source_layer: "section_heading", physical_pdf_page: 51, printed_page: 33, associated_chunk_id: "body-chunk" });
  });

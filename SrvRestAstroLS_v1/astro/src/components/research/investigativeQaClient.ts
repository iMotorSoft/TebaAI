...
export interface ResearchRequest { question: string; phase?: "legacy" | "interpret" | "analyze"; interpretation_id?: string; supersedes_interpretation_id?: string; idempotency_key?: string; scope_family?: string | null; scope_edition?: string | null; scope_document_sha256?: string | null; scope_source_work?: string | null; scope_source_lesson?: number | null; conversation: { conversation_id: string | null; turn_id: string | null; history: Array<{ question: string }> }; works: string[]; languages: Language[]; include_thematic: boolean; include_audit: false; min_evidence: "literal"; max_hits_per_work: number; return_markdown: true; return_json: true; ai: { enabled: true; model: "openai_gpt-5.4-nano" }; }
...
export const SOURCE_LAYERS = ["rebbe_lesson_text", "biblical_quote_in_lesson", "rabbinic_quote_in_lesson", "editorial_translation", "editorial_commentary", "editorial_note", "footnote", "source_reference", "section_heading", "page_heading", "marginal_reference", "introduction", "unknown"] as const;
...
function normalizeHits(value: unknown): Hit[] {
  return array<unknown>(value).map((item) => {
    ...
    const literalMatchKind = ["none", "exact_phrase", "normalized", "no_niqqud", "single_term", "semantic", "body_literal_exact", "footnote_literal_exact", "printed_reference_exact", "printed_reference_normalized", "heading_normalized", "heading_accent_folded", "literal_search_normalized", "search_normalized", "english_name_exact", "english_name_normalized", "english_name_partial", "english_name_all_tokens_ordered", "english_name_all_tokens_proximity", "hebrew_exact_normalized", "hebrew_all_tokens_ordered", "hebrew_all_tokens_proximity", "named_topic_exact", "named_topic_normalized", "named_topic_alias", "named_topic_translation", "named_topic_hebrew_equivalent", "named_topic_partial", "structural_heading_exact", "structural_heading_normalized", "structural_heading_accent_folded", "structural_heading_all_tokens_ordered", "structural_heading_all_tokens_proximity", "structural_heading_partial"].includes(String(hit.literal_match_kind)) ? hit.literal_match_kind : "none";
    ...
    return {
      ...
      metadata_warnings: array<string>(hit.metadata_warnings),

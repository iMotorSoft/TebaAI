...
export interface ResearchResponse { status: "ok" | "partial" | "no_evidence"; intent?: ResearchIntent; target_evidence_id?: string | null; same_primary_evidence?: boolean | null; interpretation?: QueryInterpretation; query_resolution?: QueryResolution; answer_text: string; answer_markdown: string; summary: string; conversation: { conversation_id: string | null; turn_id: string | null; resolved_context?: string[] }; works_consulted: string[]; hits: Hit[]; claims: Claim[]; primary_evidence_ids: string[]; evidence_counts: EvidenceCounts; evidence_matrix: MatrixRow[]; cross_corpus_matrix: MatrixRow[]; warnings: string[]; not_found: string[]; execution: Record<string, unknown>; }
...
export function normalizeResearchResponse(value: unknown): ResearchResponse {
  ...
  return { status, intent, target_evidence_id: typeof raw.target_evidence_id === "string" ? raw.target_evidence_id : null, same_primary_evidence: typeof raw.same_primary_evidence === "boolean" ? raw.same_primary_evidence : null, interpretation, query_resolution: queryResolution, answer_text: answerText, answer_markdown: answerMarkdown, summary: string(raw.summary), conversation: { conversation_id: typeof conversation.conversation_id === "string" ? conversation.conversation_id : null, turn_id: typeof conversation.turn_id === "string" ? conversation.turn_id : null, resolved_context: array<string>(conversation.resolved_context) }, works_consulted: array<string>(raw.works_consulted), hits, claims, primary_evidence_ids: [...new Set(primaryIds)], evidence_counts: evidenceCounts, evidence_matrix: normalizeMatrix(raw.evidence_matrix), cross_corpus_matrix: normalizeMatrix(raw.cross_corpus_matrix), warnings: array<string>(raw.warnings), not_found: array<string>(raw.not_found), execution: raw.execution && typeof raw.execution === "object" ? raw.execution as Record<string, unknown> : {} };
}

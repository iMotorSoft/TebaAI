...
function normalizeHits(value: unknown): Hit[] {
  return array<unknown>(value).map((item) => {
    ...
    const literalMatchKind = ["none", "exact_phrase", "normalized", "no_niqqud", "single_term", "semantic", "named_topic_exact", "named_topic_normalized", "named_topic_alias", "named_topic_translation", "named_topic_hebrew_equivalent", "named_topic_partial"].includes(String(hit.literal_match_kind)) ? hit.literal_match_kind : "none";
